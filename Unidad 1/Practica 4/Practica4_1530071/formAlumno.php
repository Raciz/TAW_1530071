...
class form
{
    ...
    public function validar()
    {
        ...
        if(!$this->error)
        {
            ...
            fputs($file,"\n$this->nombre,$this->matricula,$this->carrera,$this->email,$this->telefono");
            fclose($file);
    ...
    public function validarMatricula()
    {
        ...
        if (empty($this->matricula)) 
        {
            ...
            $this->errorArray[0] = "La matricula es requerira";
            $this->error = true;
        } 
        else 
        {
            //se guarda el valor despues de ser enviado a test_input
            $this->matricula = $this->test_input($this->matricula);
            ...
            if (!is_numeric($this->matricula)) 
            {
                ...
                //se guarda el error y error pasa a ser true
                $this->errorArray[0] = "Matricula Invalida";
                $this->error = true;
            }
        }
    }
    ...
    public function validarNombre()
    {
        ...
        if (empty($this->nombre)) 
        {
            //en caso de que este vacia se genera un error
            //se guarda el error y error pasa a ser true
            $this->errorArray[1] = "El nombre es requeriro";
            $this->error = true;
        } 
        else 
        {
    ...
    public function validarEmail()
    {
        ...
        if (!empty($this->email))  
        {
            ...
            //se erifica que se ingreso un email valido
            if (!filter_var($this->email, FILTER_VALIDATE_EMAIL)) 
            {
                //en caso de que se ingro un email invalido 
                //se guarda el error y error pasa a ser true
                $this->errorArray[3] = "Formato de email invalido";
                $this->error = true;
            }
        }
    }
    ...
    public function validarTelefono()
    {
        ...
        if(!empty($this->telefono))  
        {
            ...
            if (!is_numeric($this->telefono)) 
            {
                //en caso de que se haya introducido otra cosa
                //se guarda el error y error pasa a ser true
                $this->errorArray[4] = "Telefono Invalido";
                $this->error = true;
            }
        }
    }
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $formulario->matricula = $_POST["matricula"];
    $formulario->carrera = $_POST["carrera"];
    $formulario->email = $_POST["email"];
    $formulario->telefono = $_POST["tel"];
                ...
                <div class="section-container tabs" data-section>
                    <section class="section">
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                            ...
                            Matricula *: <input type="text" name="matricula" value="<?php echo $formulario->matricula;?>">
                            ...
                            <br>
                            Email: <input type="email" name="email" value="<?php echo $formulario->email;?>">
                            <?php if(!empty($formulario->errorArray[3])) echo "<span class='error'>*".$formulario->errorArray[3]."</span> ";?>
                            <br>
                            Telefono: <input type="text" name="tel" value="<?php echo $formulario->telefono;?>">
                            <?php if(!empty($formulario->errorArray[4])) echo "<span class='error'>*".$formulario->errorArray[4]."</span> ";?>
                            <br>
